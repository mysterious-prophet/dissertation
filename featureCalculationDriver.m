...
function imgs_features = featureCalculationDriver(img_datastore, ...
    num_imgs, bin_theta_low, bin_theta_upp, bin_mask_erode, norm, func_name, ...
    l_max, n_max, rho, frame_style, kernel_style, filter_name, nonlin_trans, features)
    
    imgs_features = [];
    if(num_imgs > 0)
        imgs_features = getImagesFeatures(img_datastore, num_imgs, bin_theta_low, ...
            bin_theta_upp, bin_mask_erode, norm, func_name, l_max, n_max, ...
            rho, frame_style, kernel_style, filter_name, nonlin_trans, features);
    end
end
...
function[imgs_features] = getImagesFeatures(img_datastore, num_imgs, bin_theta_low, ...
    bin_theta_upp, bin_mask_erode, norm, func_name, l_max, n_max, rho, frame_style, ...
    kernel_style, filter_name, nonlin_trans, features)

    [num_funcs, num_thetas, num_rhos, num_filts, num_kers, num_trans, num_invars, num_features] = ...
        getNums(func_name, bin_theta_low, rho, filter_name, kernel_style, nonlin_trans, n_max, l_max, features);

    imgs_features = NaN(num_imgs, num_funcs, num_thetas, num_rhos, num_filts, num_kers, num_trans, num_invars, num_features);
    rand_ind = randperm(size(img_datastore.Folders, 1));
    ...
    for i = 1:num_imgs
        for j = 1:num_funcs
            for k = 1:num_thetas
                [X, spect_bool] = loadImage(img_datastore.Folders{rand_ind(i)});
                if(spect_bool)
...
%                 sliceViewer(X);
                ...
                for l = 1:num_rhos
                   bin_mask = calcBinMask(X, bin_theta_low(k), bin_theta_upp(k), bin_mask_erode, rho(l));
                   invars = calcInvars(X, func_name(j), frame_style, l_max, n_max, rho(l));
                   for m = 1:num_filts
                        if(strcmp('noFilt', filter_name(m)))
                            imgs_features(i, j, k, l, m, 1, 1, :, :) = calcImageFeatures(invars, bin_mask, bin_theta_low(k), ...
                            ...
                            for n = 2:num_kers
                                for p = 1:num_trans
                                    imgs_features(i, j, k, l, m, n, p, :, :) = calcImageFeatures(invars, bin_mask, bin_theta_low(k), ...
                                        func_name(j), l_max, n_max, rho(l), frame_style,...
