...
function results = testDriver(conf_num)  
    ...
    if(conf.calc_image_features)
        ...
        fprintf("\t\t  Lower binary mask threshold(s): %s\n", getArrayString(conf.bin_theta_low));
        fprintf("\t\t  Upper binary mask threshold(s): %s\n", getArrayString(conf.bin_theta_upp));
        ...
        results.pos_imgs_features = featureCalculationDriver(conf.pos_img_datastore, ...
            conf.num_pos_imgs, conf.bin_theta_low, conf.bin_theta_upp, conf.bin_mask_erode, conf.spect_norm, conf.func_name, ...
        ...
        results.neg_imgs_features = featureCalculationDriver(conf.neg_img_datastore, ...
            conf.num_neg_imgs, conf.bin_theta_low, conf.bin_theta_upp, conf.bin_mask_erode, conf.spect_norm, conf.func_name, ...
            conf.l_max, conf.n_max, conf.rho, conf.frame_style, conf.kernel_style, conf.filter_name, conf.nonlin_trans, conf.features);
        neg_time = toc;
    ...
    if(conf.test_stat_importance)
        ...
        [results.pos_imgs_features_signif, results.neg_imgs_features_signif, results.ranksum_stat_importance_all,...
            ...
            statTestingDriver(conf.func_name, conf.bin_theta_low, conf.bin_theta_upp, conf.rho, conf.filter_name, conf.kernel_style,  ...
    ...
    if(conf.whiten_data)
        ...
        [results.X_train, results.X_train_whit] = dataWhiteningDriver(results.pos_imgs_features, results.neg_imgs_features, ...
            conf.func_name, conf.bin_theta_low, conf.bin_theta_upp, conf.rho, conf.filter_name, conf.kernel_style, conf.nonlin_trans, ...
    ...
    if(conf.class_images)
        ...
        [results.class_stats_leave1_triv, results.signif_feats_class_stats_leave1, results.class_stats_stratkfold_triv] = ...
                ...
                conf.k_fold, conf.train_params_triv, conf.num_pos_imgs, conf.num_neg_imgs, conf.func_name, conf.bin_theta_low, ...
                conf.bin_theta_upp, conf.rho, conf.filter_name, conf.kernel_style, conf.nonlin_trans, conf.n_max, conf.l_max, conf.features);
        class_time_triv = toc;
